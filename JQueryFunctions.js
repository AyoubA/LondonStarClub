...
$(document).ready(function() {
    $("#tabs").tabs( {
        "activate": function(event, ui) {
	 	...
	 	////PRODUCT DB TABLE
     var tableEvents = $("#DBTableEvents").dataTable( {
        "scrollY": "200px",
        "scrollCollapse": true,
        "paging": false,
        "jQueryUI": true
  ...
  tableEvents.fnAdjustColumnSizing();
	} );
	
	
	$('#DBTableEvents tbody').on( 'click', 'tr', function () {
        if ( $(this).hasClass('selectedEvents') ) {
            $(this).removeClass('selectedEvents');
			tableEvents.$('.selectedTDEvents').removeClass('selectedTDEvents');
			$('#selectedResultEvents').html("");
        }
        else {
            tableEvents.$('tr.selectedEvents').removeClass('selectedEvents');
			tableEvents.$('.selectedTDEvents').removeClass('selectedTDEvents');
            $(this).addClass('selectedEvents');
			$('tr.selectedEvents>td').addClass('selectedTDEvents');
			
			
			var elements = document.getElementsByClassName("selectedTDEvents");
			var str = "";
			for (var i = 0; i < elements.length; i++) {
			var str = str + "<td>" + elements[i].innerHTML + "</td>";
							}

			$('#selectedResultEvents').html(str);
        }
    } );
    ...
    $('#button').click( function () {
        tableEvents.row('.selected').remove().draw( false );
    } );
